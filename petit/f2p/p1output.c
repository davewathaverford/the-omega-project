 ...
 */
    ...
    if (oneof_stg ( addrp->uname_tag==UNAM_NAME ? addrp -> user.name : NULL,
	    stg, M(STGEQUIV)) && addrp->vtype != TYCHAR) {
        /* up there was M(STGARG)| -- vadik*/
...
void p1_for (dovar, init, test, inc)
expptr dovar, init, test, inc;
{
    p1put (P1_FOR);
    p1_expr (dovar);
    init=putx(init); p1_expr (init);
    test=putx(test); p1_expr (test);
    inc=putx(inc);   p1_expr (inc);
} /* p1_for */
