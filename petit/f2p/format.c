...
FILE *infile, *outfile;
{
	...
	if (class == CLMAIN)
	    ...
	    nice_printf(outfile, "! %s ", c_type_decl(proctype, 1));
...
/* Scalars are passed by reference, and arrays will have their lower bound
   ...
   exception is character lengths, which are passed by value. */
	...
	if (arg) {
		...
		if (arg->vimpltype)
			type = TYSUBR;
		else if (type == TYREAL && forcedouble)
			type = TYDREAL;

	    if (type == last_type && class == last_class && did_one)
		nice_printf (outfile, ", ");
	    else
		nice_printf(outfile, "%sformal %s ", sep, c_type_decl(type, is_ext));
	    if (class == CLPROC)
		{
		  nice_printf(outfile, "(*");
		  out_name(outfile, arg);
	    ...
	    if (arg->vdim &&
		!oneof_stg (arg, arg->vstg, M(STGEQUIV)|M(STGCOMMON)))
	        wr_ardecls(outfile, arg->vdim, 1L);
...
/* Now write out the system-generated identifiers */
	...
	for (this_var = new_vars; this_var; this_var = this_var -> nextp) {
		...
		if (did_one)
		    nice_printf (outfile, "\n");
		if(Var->vstg == STGARG)	        /* -- vadik */
		    nice_printf(outfile, "out "); 
		nice_printf (outfile, "%s ",
...
long size;
{
    ...
    for (i = 0; i < dimp -> ndim; i++) {
        if(dimp -> dims[i].lb) {
	    expr_out(outfile, dimp -> dims[i].lb);
	    nice_printf(outfile, ":");
	}
	expr_out(outfile, dimp -> dims[i].ub);
	nice_printf(outfile, "%s", i==dimp->ndim-1 ? "" : ",");
    } /* for i = 0 */
