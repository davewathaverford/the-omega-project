...
/* Handle a   complex   return type -- declare a new parameter (pointer to
   a complex value) */
		...
		if (!xretslot[type]) {
			xretslot[type] =
				autovar(0, type, EXNULL, " ret_val", 1);
		...
		if (!xretslot[type]) {
			xretslot[type] = retslot =
				autovar(1, type, EXNULL, " ret_val", 1);
				/* the blank is for use in out_addr */
		        }
		np->vstg = STGAUTO;
		}
...
chainp list;
{
	...
	if (namep -> vdim) {
	    nd = namep -> vdim -> ndim;
	    /* 2->4  -- vadik */
	    size = sizeof(int) + (3 + 4*nd) * sizeof (expptr);
...
Addrp autovar(nelt0, t, lengp, name, retval)
...
struct Dims dims[ ];
{
	...
	/* manual allocation: had to correct -- vadik 2->4 */
	v->vdim = p = (struct Dimblock *)
	    ckalloc( sizeof(int) + (3+4*nd)*sizeof(expptr) ); 
	...
	else
	{
		...
		p->baseoffset = (expptr) autovar(1, tyint, EXNULL, buf, 0);
		p->basexpr = q;
		v->vdimfinish = 1;
	}
	doin_setbound = 0;
